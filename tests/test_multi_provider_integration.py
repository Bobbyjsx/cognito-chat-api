...
def test_create_default_provider_registry_bedrock_claude():
    ...
    assert "bedrock_claude" in registry._providers
    assert "bedrock" in registry._providers

    claude_provider = registry.get("anthropic")
    assert isinstance(claude_provider, ClaudeProvider)
    assert claude_provider.api_key == "test-bedrock-key"
    assert claude_provider.aws_region == "eu-west-1"
